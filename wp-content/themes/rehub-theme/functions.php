...
if ( !defined( 'RH_MAIN_THEME_VERSION' ) ) {
	define('RH_MAIN_THEME_VERSION', '13.2');
}
...
$rehub_options = [
	'tf_username'      => 'GenesisforWP',
	'tf_purchase_code' => 'GenesisforWP',
	'tf_support_date'  => '26-06-2025',
];
...
function rehub_framework_register_scripts() {
	...
	wp_register_style('custum-style', get_template_directory_uri() . '/css/custum-style.css', array(), RH_MAIN_THEME_VERSION);
	...
	wp_register_script( 'zeroclipboard', get_template_directory_uri() . '/js/clipboard.min.js', array('jquery'), '2.0.7' );
	...
	wp_register_script('rehubuserlogin', get_template_directory_uri() . '/js/userlogin.js', array('jquery', 'rhpgwmodal', 'rehub'), '1.0', true);
	wp_register_script('rehubwoo', get_template_directory_uri() . '/js/woocommerce.js', array('jquery', 'rehub', 'rhniceselect'), RH_MAIN_THEME_VERSION, true);
	wp_register_script('rhcommentsort', get_template_directory_uri() . '/js/commentsort.js', array('jquery', 'rehub'), '1.0', true);
	...
	wp_register_script('owlinit', get_template_directory_uri() . '/js/owlinit.js', array('jquery', 'owlcarousel'), '1.0', true);
	...
	wp_register_script('typehead', get_template_directory_uri() . '/js/typehead.js', array('jquery'), '0.10.6', true);
	wp_register_script( 'rehubtablechart', get_template_directory_uri() . '/js/tablechart.js', array('jquery'), '1.3', true );
	wp_register_script( 'rehubcompare', get_template_directory_uri() . '/js/comparechart.js', array('jquery', 'rehubtablechart'), '1.3', true );
	...
	wp_register_script( 'gsapinit', get_template_directory_uri() . '/js/gsap-init.js', array('jquery','gsap'), '1.7', true );
...
function rehub_enqueue_scripts() {
	...
	if (rehub_option('rehub_ajax_search')) {wp_enqueue_script( 'rehubajaxsearch' );}
			...
			if ( has_post_thumbnail($post->ID) ){
				...
				if ( $attributes ) {
					...
					if(!empty($attributes['max_height'])){
						...
						$max_height = ' style="max-height:'.$attrmaxheight.'"';
					}else{
						$max_height = '';
					}
					$output = sprintf(
						'<img src="%s" data-src="%s" alt="%s" class="lazyload %s" data-large_image="%s" data-large_image_width="%s" data-large_image_height="%s"%s>',
						esc_url( $transparent ),
						esc_url( $image_url ),
						esc_attr( $alt ),
						esc_attr( $css_class ),
	...
	if (is_single() && !is_singular('product')) {
		...
		if(vp_metabox('rehub_post.rehub_framework_post_type') == 'review'){
			...
			elseif(rehub_option('type_schema_review') == 'user' && rehub_option('type_user_review') == 'simple'){
				...
				$userAverage = (!empty($rate) && !empty($count)) ? (($rate/$count)/5)*10 : 10;
				if (!$count) $count = 1;
				$jsonload["aggregateRating"] = array(
					"@type" => "AggregateRating",
      				"worstRating" => "1",
...
/*function rehub_admin_error_notice() {
    ...
    if ( ! get_user_meta($user_id, 'ignore_notices_rehub117') ) {
		//if ($page=='rehub' || $page=='rehub-support' || $page=='rehub-plugins' || $page=='rehub-demos' || $page=='vpt_option' ) {
			$class = "error";
			$message = 'This is major update of theme. All sites will be affected by changed code. Please read important information <a href="http://rehubdocs.wpsoul.com/docs/rehub-theme/for-developers/11-7-update-of-theme/" target="_blank">Rehub 11.7 version</a> which can affect your site. Also, if you have issues in theme option - update BROWSER cache.' ;
...
function rehub_nag_ignore() {
    ...
    if ( isset($_GET['rehub_nag_ignore']) && '0' == $_GET['rehub_nag_ignore'] ) {
        add_user_meta($user_id, 'ignore_notices_rehub117', 'true', true);           
	}
}*/