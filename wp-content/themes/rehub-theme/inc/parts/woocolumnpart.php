    ...
    <?php if (rehub_option('woo_btn_disable') != '1'):?>
        <div class="woo_column_btn text-center">   
            ...
            <?php elseif($countoffers == 1 && !empty($itemsync['url'])):?>
                ...
                <a href="<?php echo esc_url($ceofferurl);?>" data-product_id="<?php echo esc_attr( $product->get_id() );?>" data-product_sku="<?php echo esc_attr( $product->get_sku() );?>" class="re_track_btn fontbold rehub-btn-font rehub-main-color product_type_external" target="_blank" rel="nofollow sponsored">
            ...
            <?php elseif ( $product->add_to_cart_url() !='') : ?>               
                <?php  echo apply_filters( 'woocommerce_loop_add_to_cart_link',
                        sprintf( '<a href="%s" data-product_id="%s" data-product_sku="%s" class="re_track_btn fontbold rehub-btn-font rehub-main-color %s %s product_type_%s"%s %s>%s</a>',
