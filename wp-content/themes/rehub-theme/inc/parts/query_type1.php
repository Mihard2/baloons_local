	...
	<div class="magazinenews position-relative mb30 clearfix">	 
		<div class="border-grey-bottom flowhidden pb15 rh-flex-center-align">	
		    ...
		    <div class="magazinenews-desc rh-flex-grow1">
			    <h3 class="mt0 mb20 rem-h3"><a href="<?php the_permalink();?>"><?php $title = get_the_title(); $title = explode(" ", $title, 3); echo '<span class="rehub-main-color">'.$title[0].' '.$title[1].'</span> '.$title[2];?></a></h3>
		    ...
		    <div class="magazinenews-img width-250 mobswidth-150 pl20 rh-flex-right-align">
		        <figure class="height-150 img-width-auto text-center"> 
			        ...
			        <a href="<?php the_permalink();?>">
						<?php WPSM_image_resizer::show_static_resized_image(array('thumb'=> true, 'width'=> 220, 'height'=> 145, 'no_thumb_url' => get_template_directory_uri() . '/images/default/noimage_336_220.png'));?>
			        </a>
		        </figure>
		    </div>		            
	    </div>      
...
<?php else:?>
	<div class="news-community clearfix<?php echo rh_expired_or_not($post->ID, 'class');?>">
	    ...
	    <?php if(rehub_option('rehub_enable_expand') == 1):?>
	    	<?php wp_enqueue_script( 'rh-get-full-content', get_template_directory_uri() . '/js/getfullcontent.js', array( 'jquery', 'rehub' ), 1.0, true );?>
