...
function wpsm_gallery_carousel( $atts, $content = null ) {
	wp_enqueue_script('owlcarousel'); wp_enqueue_script('owlinit');
	extract(shortcode_atts(array(
		'ids' => '',
		'prettyphoto' => '',
		'title'=> '',
...
function wpsm_topcharts_shortcode( $atts, $content = null ) {
	...
	if(isset($atts['id']) && $atts['id']):
        ...
        <?php wp_enqueue_script('carouFredSel'); wp_enqueue_script('touchswipe'); wp_enqueue_script('rehubtablechart'); ?>                                       
...
function wpsm_categorizator_shortcode( $atts, $content = null ) {
    ...
    wp_enqueue_script( 'rhcategorizator', get_template_directory_uri() . '/js/categorizator.js', array( 'jquery' ), 1.0, true );
	ob_start(); 
    ?>
	...
	elseif ($type == 'woocategory' || $type == 'postcategory') {
		...
		if( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			foreach( $terms as $term ) {
				...
				if( $term->parent == '0' ){
					$term_titles .= '<div href="'. esc_url(get_term_link($term)) .'" class="rh-heading-hover-color col_item rh-cartbox woocatbox pt15 pb15 pr15 pl15 rh-hov-bor-line product-category rh-flex-center-align '. $classitem .'">';
					if ($show_images == 1){
						$brandimageid = get_term_meta( $term->term_id, 'thumbnail_id', true ); 
						if($brandimageid){
							$brandimage = wp_get_attachment_url($brandimageid);
	...
	$output = '<div class="rh-cartbox catbox mb20">';
		...
		if ( $image ){
			...
			$output .= '<div class="rh-transition-box">';					
				if ( $disablelink != 1 && is_numeric( $term->term_id )) {
...
if( !function_exists('wpsm_get_add_deal_popup') ){
	function wpsm_get_add_deal_popup($atts, $content = NULL){
		...
		wp_enqueue_script( 'rh-ce-front', get_template_directory_uri() . '/js/cefrontsubmit.js', array( 'jquery', 'rehub' ), 1.0, true );
    	...
    	if($score){
    		...
    		elseif($compact == 'squarecolor'){
		    	...
		    	$out = '<div class="mb15 compact-reviewbox whitebg colored_rate_bar rh-flex-center-align border-lightgrey">';
		    		...
		    		$out .= '<div class="rev-comp-text lineheight20 ml15 mr15"><span class="rev-comp-title">'.$title.'</span>';
