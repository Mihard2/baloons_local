...
class WPSM_Woohelper{
	...
	public static function re_show_brand_tax($type='list', $height = '30'){   
        ...
        if ($type=='logo' || $type=='logoname'){
	        ...
	        if (!empty($brand_link) && ($type=='logoname' || !empty($term_brand_image))){echo '<a href="' . esc_url( $brand_link ) . '">';}
			...
			if (!empty($brand_link) && ($type=='logoname' || !empty($term_brand_image))){echo '</a>';}
        }                
	}	
...
<ul class="tabs-menu mb20 clearfix">
...
ob_start(); 
?>
<?php wp_enqueue_script('owlcarousel'); wp_enqueue_script('owlinit'); ?>
...
$center_class=($center) ? ' text-center rh-list-center': ''; 
$ajaxoffset = (int)$show + (int)$offset;
$additional_vars = array();
...
$additional_vars['maximage'] = $maximage;
