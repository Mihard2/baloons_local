...
function rh_change_breadcrumb_delimiter( $defaults ) {
	...
	$defaults['delimiter'] = '<span class="delimiter"><i class="rhicon rhi-angle-right"></i></span>';
	return $defaults;
}
...
if (rehub_option('woo_cart_place') =='1' || rehub_option('woo_cart_place') =='2' || (rehub_option('rehub_header_style') =='header_seven' && rehub_option('header_seven_cart') != '')){
	...
	function woocommerce_header_add_to_cart_fragment( $fragments ) {
		...
		<?php if (rehub_option('woo_cart_place') =='1'):?>
			<a class="cart-contents cart_count_<?php echo ''.$woocommerce->cart->cart_contents_count; ?>" href="<?php echo wc_get_cart_url(); ?>"><i class="rhicon rhi-cartfeather"></i> <?php esc_html_e( 'Cart', 'rehub-theme' ); ?> (<?php echo ''.$woocommerce->cart->cart_contents_count; ?>) - <?php echo ''.$woocommerce->cart->get_total(); ?></a>		
