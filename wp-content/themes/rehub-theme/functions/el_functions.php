...
function RH_parallax_el_elementor( $obj, $args ) {
    ...
    $obj->add_control( 'rh_gsap_trigger_field', [
        ...
        'type' => \Elementor\Controls_Manager::TEXT,
        'condition' => array(
            'rh_gsap_trigger_type' => ['custom', 'batch', 'hover'], 'rh_gsap' => 'true',
        ),
    ]);
...
function RH_el_elementor_frontend( $element) {
    if ( $element->get_settings( 'rh_gsap' ) == 'true' || $element->get_settings( 'rh_parallax_el' ) == 'true' ) {
        wp_enqueue_script('gsap');
        if($element->get_settings( 'rh_gsap_trigger_type' ) == 'custom' || empty($element->get_settings( 'rh_gsap_trigger_type' )) || $element->get_settings( 'rh_gsap_trigger_type' ) == 'batch' || $element->get_settings( 'rh_parallax_el' ) == 'true'){
            wp_enqueue_script('scrolltrigger');
        }
...
function RH_el_custom_widget_render( $content, $widget ) {
    ...
    if ( ! empty( $settings['rh_parallax_el'] )) {
        ...
        $content = '<div data-triggerscrub="1" data-triggertype="custom" class="rh-gsap-wrap" '.$widget->get_render_attribute_string( 'rhpar-wrapper' ).'>'.$content. '</div>';
    }
